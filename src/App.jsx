...
const USERS = {
  "2254": { name: "Δημήτρης", role: "admin", finance: true },
  "0966": { name: "Γιάννης", role: "admin", finance: true },
  "1772": { name: "Τάσος", role: "admin", finance: false },
  "0838": { name: "Βασιλική", role: "crew", finance: false },
  "4556": { name: "Ιάσωνας", role: "crew", finance: false },
};
...
function FinanceView() {
  ...
  const martaRevenue = filtered.filter(c => c.vessel === "MARTA").reduce((s, c) => s + (c.total || 0), 0);
  const romviRevenue = filtered.filter(c => c.vessel === "ROMVI").reduce((s, c) => s + (c.total || 0), 0);
  const todayStr = new Date().toISOString().slice(0, 10);
  const upcomingCharters = filtered.filter(c => c.date >= todayStr);
  const completedCharters = filtered.filter(c => c.date < todayStr);
        ...
        <>
          ...
          <div className="stats-grid">
            ...
            <div className="stat-card">
              ...
              <div style={{ fontSize: "0.72rem", color: "var(--ocean)", marginTop: 4, fontWeight: 600 }}>€{martaRevenue.toFixed(0)}</div>
            </div>
            <div className="stat-card">
              <div className="stat-num">{romviCount}</div>
              <div className="stat-label">Ναύλοι Romvi</div>
              <div style={{ fontSize: "0.72rem", color: "var(--ocean-dark)", marginTop: 4, fontWeight: 600 }}>€{romviRevenue.toFixed(0)}</div>
            </div>
          </div>
          <div className="card">
          ...
          <div className="card">
            ...
            {CLIENT_TYPES.map(ct => {
              const ctCharters = filtered.filter(c => {
                if (ct === "Direct booking") return c.client_type === "Direct booking" || c.client_type === "Ιδιώτης";
                return c.client_type === ct;
              });
...
export default function App() {
  ...
  const handlePin = () => {
    ...
    if (user) {
      ...
      setCanFinance(user.finance);
      setPinError("");
      setTab(user.role === "crew" ? "daily" : "dashboard");
      setStep("app");
    } else {
      setPinError("Λάθος PIN. Δοκίμασε ξανά.");
    }
    setPin("");
  };

  const handleLogout = () => { setStep("pin"); setRole(null); setUserName(""); setPin(""); setCanFinance(false); setTab("daily"); };
  ...
  const navItems = [
    ...
    ...(canFinance ? [{ id: "finance", label: "Οικονομικά", icon: "finance" }] : []),
    ...(role !== "finance" ? [
